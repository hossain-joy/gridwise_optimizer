...
[📖 Overview](#-overview) · [🏗️ Architecture](#️-architecture) · [📐 Optimization Model](#-optimization-model) · [💡 Complete Example](#-complete-example) · [🔌 API Specification](#-api-specification) · [🎨 Interactive Dashboard](#-interactive-dashboard) · [🛠️ Technology Stack](#️-technology-stack) · [📁 Project Structure](#-project-structure) · [🚀 Quick Start](#-quick-start) · [🐳 Docker](#-docker-deployment) · [🧪 Testing & Benchmarks](#-testing--benchmarks) · [👥 Competition Details](#-competition-details)
...
| **☀️ Solar PV** | Zero marginal cost, forecast-dependent generation profile | Primary clean generation source |
...
> 🗣️ *"Facilities will wash the rooftop solar panels from noon until 2 PM. During cleaning, usable solar should be treated as roughly 25% of the forecast."*
...
Translates natural language notes into structured machine-actionable JSON directives using **Google AI Studio (Gemini 2.5 Flash)** with prompt caching and deterministic regex NLP fallback.
...
### 2. 🛡️ Deterministic Guardrails
- ⏱️ Validates all time ranges to strictly start-inclusive, end-exclusive `[start, end)` subsets within `[0, 23]`.
...
### 3. ⚙️ Coin-OR CBC Linear Optimization
...
### 2. ☀️ Solar Availability Constraints
...
**Operator Notes:**
1. 🗣️ *"Facilities will wash the rooftop solar panels from noon until 2 PM. During cleaning, usable solar should be treated as roughly 25% of the forecast."*
2. 🗣️ *"The sports office moved next month's registration deadline."*
...
- 🖥️ **Dribbble Modern SaaS Layout:** Styled with dark forest green (`#052e16`) theme, clean metric cards, and responsive sidebar.
...
- ⌨️ **Keyboard Shortcuts:** `⌘K` / `Ctrl+K` (Scenario Switcher), `Ctrl+Enter` (Run Optimization).
...
| **🤖 AI / NLP Interpretation** | **Google AI Studio (Gemini 2.5 Flash)** | Semantic parsing of unstructured natural language shift notes |
| **🛡️ Validation & Schemas** | **Pydantic v2** | Strict deterministic type validation and schema enforcement |
| **📐 Optimization Modeling** | **PuLP** | Mathematical linear programming modeling framework |
| **⚙️ Underlying LP Solver** | **Coin-OR CBC** | High-performance open-source simplex & branch-and-bound solver |
...
| **☁️ Hosting & Cloud** | **Render Cloud Platform** | Zero-downtime auto-deployments linked to GitHub `main` |
...
│   ├── llm_interpreter.py       # Google Gemini LLM translation, cache & NLP fallback
...
│   └── architecture.png         # High-resolution multi-stage pipeline diagram
├── tests/
...
```
- 🖥️ **Interactive Dashboard:** `http://localhost:8000/` or `http://localhost:8000/demo`
